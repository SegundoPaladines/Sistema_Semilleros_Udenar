...
<div class="container">
                ...
                <tbody>
                    ...
                    @foreach($semilleristas_libres as $s)
                        <tr>
                            ...
                            <td class="centered-cell">{{ app('App\Http\Controllers\Coordinador\CoordinadorController')->obtenerNombrePersona($s->num_identificacion) }}</td>
                            <td class="centered-cell">{{ app('App\Http\Controllers\Coordinador\CoordinadorController')->obtenerCorreoUsuario($s->num_identificacion) }}</td>
                            <td class="centered-cell">{{ $s->semestre }}</td>
                            <td class="centered-cell">
                                <center>
                                    <a style="margin: 3px;" href="{{ route('vincular_sem', $s->num_identificacion) }}" class="btn btn-success btn-sm">Vincular</a>
                                </center>
                            </td>
                        </tr>
                        @php
    ...
    @if (session('vinculacionFallida'))
        <script>
            document.addEventListener('DOMContentLoaded', function() {
                mostrarAlertaRegistroExitoso("¡El semillerista no existe!","Vinculacion Fallida", false);
            });
        </script>
    @endif
